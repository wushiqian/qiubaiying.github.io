...
手指位于A位置：表明手指是对裁剪框的四个角某一个角进行拉伸缩放操作。此时随着手指的移动，裁剪框的四条边的长度都会发生变化：要么同时缩小，要么同时放大。
...
正如上图所示一个裁剪框有四条边，在cropper的实现中将这四条边用一个对象来表示（枚举类）：
...
```java
     ...
     */
    public static CropWindowEdgeSelector getPressedHandle(float x,float y,float left,float top, float right,float bottom,float targetRadius) {
